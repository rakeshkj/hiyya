...
class BxTeamsSearchResult extends BxDolTwigSearchResult
{
    var $aCurrent = array(
        'name' => 'bx_teams',
        'title' => '_bx_teams_page_title_browse',
        'table' => 'bx_teams_main',
        ...
        'restriction' => array(
            ...
            'public' => array('value' => '', 'field' => 'allow_view_team_to', 'operator' => '='),
        ),
    ...
    function BxTeamsSearchResult($sMode = '', $sValue = '', $sValue2 = '', $sValue3 = '')
    {
        switch ($sMode) {

            case 'pending':
                if (false !== bx_get('bx_teams_filter'))
                    $this->aCurrent['restriction']['keyword'] = array('value' => process_db_input(bx_get('bx_teams_filter'), BX_TAGS_STRIP), 'field' => '','operator' => 'against');
                $this->aCurrent['restriction']['activeStatus']['value'] = 'pending';
                $this->sBrowseUrl = "administration";
                $this->aCurrent['title'] = _t('_bx_teams_page_title_pending_approval');
            ...
            case 'search':
                ...
                $this->aCurrent['title'] = _t('_bx_teams_page_title_search_results') . ' ' . (is_array($sValue2) ? implode(', ',$sValue2) : $sValue2) . ' ' . $sValue;
                unset($this->aCurrent['rss']);
                break;

            case 'user':
                $iProfileId = $GLOBALS['oBxTeamsModule']->_oDb->getProfileIdByNickName ($sValue, false);
                ...
                $this->aCurrent['title'] = ucfirst(strtolower($sValue)) . _t('_bx_teams_page_title_browse_by_author');
                if (bx_get('rss')) {
            ...
            case 'joined':
                ...
                $this->aCurrent['title'] = ucfirst(strtolower($sValue)) . _t('_bx_teams_page_title_browse_by_author_joined_teams');
            ...
            case 'admin':
                ...
                $this->aCurrent['title'] = _t('_bx_teams_page_title_admin_teams');
                break;
            ...
            case 'category':
                ...
                $this->aCurrent['title'] = _t('_bx_teams_page_title_browse_by_category') . ' ' . $sValue;
                break;
            ...
            case 'tag':
                ...
                $this->aCurrent['title'] = _t('_bx_teams_page_title_browse_by_tag') . ' ' . $sValue;
                break;

            case 'recent':
                $this->sBrowseUrl = 'browse/recent';
                $this->aCurrent['title'] = _t('_bx_teams_page_title_browse_recent');
                break;

            case 'top':
                $this->sBrowseUrl = 'browse/top';
                $this->aCurrent['sorting'] = 'top';
                $this->aCurrent['title'] = _t('_bx_teams_page_title_browse_top_rated');
                break;

            case 'popular':
                $this->sBrowseUrl = 'browse/popular';
                $this->aCurrent['sorting'] = 'popular';
                $this->aCurrent['title'] = _t('_bx_teams_page_title_browse_popular');
                break;

            case 'featured':
            $this->aCurrent['restriction']['featured'] = array('value' => 1, 'field' => 'featured', 'operator' => '=');
                $this->sBrowseUrl = 'browse/featured';
                $this->aCurrent['title'] = _t('_bx_teams_page_title_browse_featured');
                break;
        ...
        $this->aCurrent['paginate']['perPage'] = $oMain->_oDb->getParam('bx_teams_perpage_browse');
        ...
        if (bx_get('rss')) {
            ...
            $this->aCurrent['paginate']['perPage'] = $oMain->_oDb->getParam('bx_teams_max_rss_num');
        }

        bx_import('Voting', $oMain->_aModule);
        $oVotingView = new BxTeamsVoting ('bx_teams', 0);
    ...
    function getAlterOrder()
    {
        if ($this->aCurrent['sorting'] == 'last') {
            $aSql = array();
            $aSql['order'] = " ORDER BY `bx_teams_main`.`created` DESC";
            return $aSql;
        } elseif ($this->aCurrent['sorting'] == 'top') {
            $aSql = array();
            $aSql['order'] = " ORDER BY `bx_teams_main`.`rate` DESC, `bx_teams_main`.`rate_count` DESC";
            return $aSql;
        } elseif ($this->aCurrent['sorting'] == 'popular') {
            $aSql = array();
            $aSql['order'] = " ORDER BY `bx_teams_main`.`fans_count` DESC, `bx_teams_main`.`views` DESC";
            return $aSql;
        }
        return array();
    }
