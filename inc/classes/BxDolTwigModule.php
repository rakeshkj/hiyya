...
class BxDolTwigModule extends BxDolModule
{
    ...
    //PG View by Rakesh Jha
	function _actionPgview ($sUri, $sMsgPendingApproval)
    {
        $this->_oTemplate->pageStart();
        bx_import ('PagePgview', $this->_aModule);
        $sClass = $this->_aModule['class_prefix'] . 'PagePgview';
		$aDataEntry = $this->_oDb->getPgEntryByUri($sUri);
        $oPage = new $sClass ($this, $aDataEntry);
        if ($aDataEntry[$this->_oDb->_sFieldStatus] == 'pending') {
            $aVars = array ('msg' => $sMsgPendingApproval); // this product is pending approval, please wait until it will be activated
            echo $this->_oTemplate->parseHtmlByName ('pending_approval_plank', $aVars);
	...
	//Added for playground edit functionality By Rakesh
	function _actionEditPlayground ($iEntryId, $sTitle)
    {
        $iEntryId = (int)$iEntryId;
        if (!($aDataEntry = $this->_oDb->getEntryByIdPlayground($iEntryId))) {
            $this->_oTemplate->displayPageNotFound ();
            return;
        }
        ...
        $GLOBALS['oTopMenu']->setCustomSubHeader($aDataEntry[$this->_oDb->_sFieldTitle]);
        $GLOBALS['oTopMenu']->setCustomVar($this->_sPrefix.'_view_uri', $aDataEntry[$this->_oDb->_sFieldUri]);
        $GLOBALS['oTopMenu']->setCustomSubHeaderUrl(BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'pgview/' . $aDataEntry[$this->_oDb->_sFieldUri]);
        $GLOBALS['oTopMenu']->setCustomBreadcrumbs(array(
            _t('_'.$this->_sPrefix) => BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'home/',
            $aDataEntry[$this->_oDb->_sFieldTitle] => BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'pgview/' . $aDataEntry[$this->_oDb->_sFieldUri],
            $sTitle => '',
        ));
        ...
        bx_import ('PlaygroundFormEdit', $this->_aModule);
        $sClass = $this->_aModule['class_prefix'] . 'PlaygroundFormEdit';
        $oForm = new $sClass ($this, $aDataEntry[$this->_oDb->_sFieldAuthorId], $iEntryId, $aDataEntry);
        if (isset($aDataEntry[$this->_oDb->_sFieldJoinConfirmation]))
            $aDataEntry[$this->_oDb->_sFieldJoinConfirmation] = (int)$aDataEntry[$this->_oDb->_sFieldJoinConfirmation];
        ...
        if ($oForm->isSubmittedAndValid ()) {
            ...
            if ($oForm->update ($iEntryId, $aValsAdd)) {
                ...
                $this->onEventChanged ($iEntryId, $sStatus);
                header ('Location:' . BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'pgview/' . $aDataEntry[$this->_oDb->_sFieldUri]);
                exit;
	...
	function _actionDeletePlayground ($iEntryId, $sMsgSuccess)
    {
        header('Content-type:text/html;charset=utf-8');

        $iEntryId = (int)$iEntryId;
        if (!($aDataEntry = $this->_oDb->getEntryByIdPlayground($iEntryId, $this->_iProfileId, $this->isAdmin()))) {
        ...
        if ($this->_oDb->deletePgEntryByIdAndOwner($iEntryId, $this->_iProfileId, $this->isAdmin())) {
            $this->isAllowedDelete($aDataEntry, true); // perform action
            $this->onEventDeleted ($iEntryId, $aDataEntry);
            $sRedirect = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri();
